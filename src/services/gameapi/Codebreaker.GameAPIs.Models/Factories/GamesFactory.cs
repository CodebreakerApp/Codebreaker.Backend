...
public static class GamesFactory
{
    ...
    public static Game CreateGame(GameType gameType, string playerName)
    {
        ...
        static SimpleGame Get6x4SimpleGame(GameType gameType, string playerName) =>
        ...
        static ColorGame Get6x4Game(GameType gameType, string playerName) =>
            new(Guid.NewGuid(), gameType, playerName, DateTime.Now, 4, 12)
            {
                Codes = Enumerable.Range(0, 4).Select(i => new ColorField(GetRandomValue(s_colors6))).ToArray()
            };

        static ColorGame Get8x5Game(GameType gameType, string playerName) =>
            new(Guid.NewGuid(), gameType, playerName, DateTime.Now, 5, 14)
            {
                Codes = Enumerable.Range(0, 5).Select(i => new ColorField(GetRandomValue(s_colors8))).ToArray()
            };

        static ShapeGame Get5x5x4Game(GameType gameType, string playerName) =>
        ...
        {
            GameType.Game6x4Mini => Get6x4SimpleGame(gameType, playerName),
